...
class ZIndexTestHelper {
  ...
  async checkZIndexHierarchy(
    elements: Array<{ selector: string; expectedLevel: keyof typeof this.zIndexLevels }>
  ) {
    const results: Array<{
      selector: string;
      actualZIndex: number;
  ...
  async checkElementsStackingOrder(
    topElementId: string,
...
test.describe('Modal Z-Index Hierarchy Tests', () => {
  ...
  test('should stack multiple modals correctly', async ({ page }) => {
    ...
    if (await assetManagerButton.isVisible()) {
      ...
      const configButtonInModal = page.locator(
        'button:has-text("Config"), button:has-text("Settings")'
      );
      ...
      if ((await configButtonInModal.count()) > 0) {
        ...
        if (modalCount > 1) {
          const secondModalZIndex = await zIndexHelper.getComputedZIndex(
            '.fixed.inset-0:nth-child(2)'
          );
...
test.describe('Dropdown and Popover Z-Index Tests', () => {
    ...
    if (await assetManagerButton.isVisible()) {
      ...
      if (dropdownCount > 0) {
        ...
        if ((await dropdownMenu.count()) > 0) {
          const dropdownZIndex = await zIndexHelper.getComputedZIndex(
            '.dropdown-menu, [role="listbox"], .z-dropdown'
          );
  ...
  test('should have correct z-index for tooltips', async ({ page }) => {
    ...
    if (triggerCount > 0) {
      ...
      if ((await tooltips.count()) > 0) {
        const tooltipZIndex = await zIndexHelper.getComputedZIndex(
          '.tooltip, [role="tooltip"], .z-tooltip'
        );
  ...
  test('should stack dropdowns above modal content but below notifications', async ({ page }) => {
    ...
    const notificationAboveModal = await zIndexHelper.checkElementsStackingOrder(
      'notification-element',
      'modal-element'
    );
    const modalAboveBackground = await zIndexHelper.checkElementsStackingOrder(
      'modal-element',
      'background-element'
    );
...
test.describe('Navigation and UI Element Z-Index Tests', () => {
  ...
  test('should have correct z-index for title bar', async ({ page }) => {
    ...
    if ((await titlebar.count()) > 0) {
      const titlebarZIndex = await zIndexHelper.getComputedZIndex(
        '.titlebar, .title-bar, .z-titlebar'
      );
  ...
  test('should have correct z-index for floating help button', async ({ page }) => {
    ...
    const floatingButton = page.locator(
      '.floating-help, [data-testid="floating-help"], .z-floating'
    );

    if ((await floatingButton.count()) > 0) {
      const floatingZIndex = await zIndexHelper.getComputedZIndex(
        '.floating-help, [data-testid="floating-help"], .z-floating'
      );
...
test.describe('Notification Z-Index Tests', () => {
  ...
  test('should have highest z-index for notifications', async ({ page }) => {
    ...
    const notifications = page.locator(
      '.notification, [role="alert"], .z-notification, [data-testid*="notification"]'
    );

    if ((await notifications.count()) > 0) {
      const notificationZIndex = await zIndexHelper.getComputedZIndex(
        '.notification, [role="alert"], .z-notification'
      );
  ...
  test('should display notifications above all other elements', async ({ page }) => {
    ...
    const notificationAboveBackground = await zIndexHelper.checkElementsStackingOrder(
      'test-notification',
      'background-element'
    );
