...
class AssetManagerPanelPage {
  ...
  // Selectors
  private readonly assetManagerButton =
...
test.describe('AssetManagerPanel Modal Tests', () => {
  ...
  test('should display portfolio health score', async ({ page }) => {
    ...
    const healthElement = page
      .locator('.text-white\\/60:has-text("Portfolio Health:")')
      .locator('+ span');
...
test.describe('AssetManagerPanel Z-Index and Overlay Tests', () => {
  ...
  test('should handle multiple modals correctly', async ({ page }) => {
    ...
    const automationButton = page.locator(
      'button:has-text("Automation"), [data-testid="automation-config"]'
    );
    ...
    if (await automationButton.isVisible()) {
      ...
      if (await automationModal.isVisible()) {
        const automationZIndex = await automationModal.evaluate(
          el => window.getComputedStyle(el).zIndex
        );
        const assetManagerZIndex = await (
          await assetManagerPage.getModalElement()
