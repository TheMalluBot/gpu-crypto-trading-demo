  ...
  async checkColorContrast() {
    const contrastResults = await this.page.evaluate(() => {
      ...
      elements.forEach(el => {
        ...
        const ratio = this.calculateContrastRatio ? this.calculateContrastRatio(color, backgroundColor) : null;
        
        if (ratio && ratio < 4.5) { // WCAG AA standard
          issues.push({
  ...
  async checkHeadingStructure() {
    ...
    for (let i = 0; i < headings.length; i++) {
      ...
      if (heading.level > previousLevel + 1) {
        issues.push(`Heading level jumps from h${previousLevel} to h${heading.level} at position ${i + 1}`);
      }
  ...
  async checkFormAccessibility() {
    const formResults = await this.page.evaluate(() => {
      ...
      requiredInputs.forEach(input => {
        ...
        const hasVisualIndicator = htmlInput.getAttribute('aria-label')?.includes('required') ||
...
test.describe('Asset Manager Panel Accessibility Tests', () => {
  ...
  test('should trap focus within modal', async ({ page }) => {
    ...
    if (await assetManagerButton.isVisible()) {
      ...
      // Test focus trapping
      const focusableElements = page.locator('button, [tabindex]:not([tabindex="-1"]), input, select');
  ...
  test('should handle modal keyboard navigation', async ({ page }) => {
    ...
    if (await assetManagerButton.isVisible()) {
      ...
      while (attempts < 10) {
        ...
        const isAssetManagerButton = await focused.evaluate(el => 
  ...
  test('should support arrow key navigation for tabs', async ({ page }) => {
    ...
    if (await assetManagerButton.isVisible()) {
      ...
      if (await firstTab.isVisible()) {
        ...
        const isTab = await focusedTab.evaluate(el => 
          el.tagName.toLowerCase() === 'button' && 
          el.closest('.flex.border-b') !== null
        );
