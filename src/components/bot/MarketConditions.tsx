      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div>
            ...
            <div className="text-white font-medium">{(marketConditions.volatility * 100).toFixed(1)}%</div>
          </div>
          <div>
            <div className="text-white/60">Trend Strength</div>
            <div className="text-white font-medium">{(marketConditions.trend_strength * 100).toFixed(1)}%</div>
          </div>
          <div>
            <div className="text-white/60">Volume</div>
            <div className="text-white font-medium">{(marketConditions.volume_profile * 100).toFixed(1)}%</div>
          </div>
          <div>
        ...
        {config.auto_strategy_enabled && (
          <div className="mt-3 flex items-center space-x-2">
            ...
            <span className="text-white/60 text-sm">- Adapting to {marketConditions.market_regime.toLowerCase()} market</span>
          </div>
        )}
      </motion.div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="text-white/60 text-sm">
                {botStatus.latest_signal.signal_type} signal detected with {(botStatus.latest_signal.strength * 100).toFixed(1)}% strength
              </div>
            </div>
          </div>
        </motion.div>
