...
const OptimizedParticleCanvas = memo(({
  ...
  const render2D = useCallback((
    ...
    const particleCountToRender = performanceMode === 'high' ? particles.length :
                                  performanceMode === 'medium' ? Math.floor(particles.length * 0.7) :
  ...
  return (
    <div className={`particle-canvas-container ${className}`}>
      ...
      <div className="performance-indicator">
        ...
        {isGPUAvailable && enableGPU && (
          <div className="gpu-indicator">GPU</div>
        )}
      </div>
    </div>
  )
