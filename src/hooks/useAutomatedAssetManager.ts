...
export const useAutomatedAssetManager = () => {
  ...
  const checkRiskAdjustments = useCallback(async (): Promise<AssetManagerActions[]> => {
    ...
    if (portfolioHealth.risk_health < 60) {
      ...
      for (const position of highRiskPositions.slice(0, 3)) {
        // Limit to top 3 riskiest
        actions.push({
  ...
  const executeActions = useCallback(async (actions: AssetManagerActions[]) => {
    ...
    for (const action of actions) {
      try {
        console.log(
          `🔄 Executing automated action: ${action.type} ${action.action} ${action.symbol}`
        );
  ...
  const executeBuyOrder = useCallback(
    async (action: AssetManagerActions): Promise<boolean> => {
      try {
  ...
  const onBotSignal = useCallback(
    async (signal: LROSignal, botStatus: BotStatus) => {
  ...
  const checkPositionSizingAdjustments = useCallback(
    async (signal: LROSignal): Promise<AssetManagerActions[]> => {
      const actions: AssetManagerActions[] = [];
      ...
      if (signal.strength > 0.8) {
        ...
        actions.push({
          ...
          symbol: 'BTCUSDT',
          action:
            signal.signal_type === 'Buy' || signal.signal_type === 'StrongBuy' ? 'BUY' : 'SELL',
  ...
  useEffect(() => {
    if (pendingActions.length > 0 && !isProcessing) {
      const mediumPriorityActions = pendingActions.filter(
        action => action.priority === 'MEDIUM' && !action.executed
      );
  ...
  return {
    ...
    stats: {
      ...
      profitActionsToday: executedActions.filter(
        action =>
          action.type === 'PROFIT_TAKING' &&
          action.timestamp.toDateString() === new Date().toDateString()
      ).length,
      rebalanceActionsToday: executedActions.filter(
        action =>
