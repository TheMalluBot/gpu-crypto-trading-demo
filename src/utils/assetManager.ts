...
export class AssetManager {
    ...
    for (const bucket of this.state.config.risk_buckets) {
      if (bucket.current_allocation > bucket.max_allocation) {
        suggestions.push({
          ...
          suggested_amount: (bucket.current_allocation - bucket.max_allocation) * this.state.config.total_capital / 100,
  ...
  public calculatePortfolioHealth(): PortfolioHealth {
    ...
    const overall_score = (allocation_health + risk_health + profit_health + correlation_health) / 4;
  ...
  private applyDynamicSizing(symbol: string, base_size: number): number {
    ...
    const correlation_multiplier = Math.max(0.7, Math.min(1.0, 1 - position.correlation_score / 100));
    
    return base_size * volatility_multiplier * correlation_multiplier;
  }

  // Enforce allocation limits
  private enforceAllocationLimits(symbol: string, position_size: number, entry_price: number): number {
  ...
  private createProfitSecuringAction(position: PositionMetrics, zone: ProfitZone): ProfitSecuringAction | null {
    switch (zone.action) {
      case 'secure':
        return {
  ...
  private getAvailableCapital(): number {
    const allocated_capital = this.state.current_positions.reduce(
      (sum, pos) => sum + (pos.position_size * pos.current_price), 0
    );
    const reserved_cash = this.state.config.total_capital * (this.state.config.reserved_cash_percentage / 100);
  ...
  private getCurrentClassAllocation(className: string): number {
    const class_positions = this.state.current_positions.filter(pos => pos.asset_class === className);
    const class_value = class_positions.reduce((sum, pos) => sum + (pos.position_size * pos.current_price), 0);
  ...
  private calculateProfitHealth(): number {
    const profitable_positions = this.state.current_positions.filter(pos => pos.unrealized_pnl > 0).length;
  ...
  private selectWorstPerformerInBucket(bucketId: string): string {
    const bucket_positions = this.state.current_positions.filter(pos => pos.risk_bucket === bucketId);
    if (bucket_positions.length === 0) return 'BTCUSDT';
    
    // Return the worst performer
    return bucket_positions.sort((a, b) => a.unrealized_pnl_percentage - b.unrealized_pnl_percentage)[0].symbol;
  }
...
export const DEFAULT_CONSERVATIVE_CONFIG: AssetManagerConfig = {
  ...
  profit_zones: [
    { level: 10, action: 'secure', percentage_to_secure: 25, remaining_percentage: 75, stop_loss_adjustment: 1 },
    { level: 20, action: 'secure', percentage_to_secure: 50, remaining_percentage: 50, stop_loss_adjustment: 5 },
    { level: 50, action: 'trail', percentage_to_secure: 0, remaining_percentage: 100, stop_loss_adjustment: 10 }
  ],
