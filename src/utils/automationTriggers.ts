...
export const PROFIT_TAKING_TRIGGERS: AutomationTrigger[] = [
  {
    ...
    check: (context) => {
      ...
      const profitPercent = ((position.entry_price - position.entry_price) / position.entry_price) * 100;
  ...
  
  {
    ...
    check: (context) => {
      ...
      const isReversal = signal.signal_type === 'StrongSell' && position.side === 'Long' ||
...
export const POSITION_SIZING_TRIGGERS: AutomationTrigger[] = [
  {
    ...
    check: (context) => {
      ...
      return context.latestSignal.strength > 0.85 && 
             (context.latestSignal.signal_type === 'StrongBuy' || context.latestSignal.signal_type === 'StrongSell');
    }
  },
...
export class AutomationTriggerEngine {
  ...
  private evaluateTriggers(triggers: AutomationTrigger[], context: AutomationContext): AutomationTrigger[] {
...
export const AutomationHelpers = {
  calculateProfitPercentage: (entryPrice: number, currentPrice: number, side: 'Long' | 'Short'): number => {
    if (side === 'Long') {
  ...
  calculateOptimalRebalanceAmount: (currentAllocation: number, targetAllocation: number, totalCapital: number): number => {
  ...
  isMarketConditionFavorable: (marketConditions: MarketConditions): boolean => {
    return marketConditions.trend_strength > 0.6 && 
