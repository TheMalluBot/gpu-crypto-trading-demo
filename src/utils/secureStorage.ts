...
class SecureStorage {
  ...
  static async retrieve(key: string): Promise<any | null> {
    try {
      ...
      const password = this.getDeviceKey();
      const salt = new Uint8Array(secureData.salt.match(/.{2}/g)?.map(byte => parseInt(byte, 16)) || []);
      const iv = new Uint8Array(secureData.iv.match(/.{2}/g)?.map(byte => parseInt(byte, 16)) || []);
      const encrypted = new Uint8Array(secureData.encrypted.match(/.{2}/g)?.map(byte => parseInt(byte, 16)) || []);
