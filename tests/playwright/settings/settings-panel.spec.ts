...
test.describe('Settings Panel', () => {
  ...
  test.describe('API Credential Input and Validation', () => {
    ...
    test('should validate Ed25519 API credentials', async ({ page }) => {
      ...
      await helpers.fillFieldSafely('#api-secret-input', INVALID_API_CREDENTIALS.ed25519.api_secret);
  ...
  test.describe('Connection Testing', () => {
    ...
    test('should reset test result when credentials change', async ({ page }) => {
      ...
      // Change API key
      await helpers.fillFieldSafely('#api-key-input', 'different_key_value_for_testing_reset_functionality');
  ...
  test.describe('Reset to Defaults', () => {
    test('should provide reset functionality', async ({ page }) => {
      ...
      const resetButton = page.locator('button:has-text("Reset"), button:has-text("Default"), [data-testid="reset-button"]');
